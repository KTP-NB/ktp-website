    ...
    <header className="absolute inset-x-0 top-0 z-50 bg-transparent">
      <nav aria-label="Global" className="flex items-center justify-between p-6 lg:px-8">
        ...
        <div className="hidden lg:flex lg:gap-x-12">
          {navigation.map((item) => (
            <Link key={item.name} href={item.href} className="text-sm font-semibold leading-6 text-white hover:bg-white hover:text-blue-950 px-3 py-2 rounded-md">
              {item.name}
            </Link>
          ))}
        </div>
          ...
          <div className="mt-6 flow-root">
            <div className="-my-6 divide-y divide-gray-700">
              ...
              <div className="py-6">
                <Link
                  href="#"
                  className="-mx-3 block rounded-lg px-3 py-2.5 text-base font-semibold leading-7 text-white hover:bg-gray-700 hover:text-indigo-300"
                >
                  Log in
                </Link>
              </div>
