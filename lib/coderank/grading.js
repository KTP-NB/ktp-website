...
export async function gradeSubmission({ language, code, tests, revealHidden = false, question = null }) {
  ...
  // Run all tests in parallel (the executor is per-request stateless).
  const settled = await Promise.allSettled(
    tests.map((t) => runOnce({ language, code, stdin: t.stdin })),
  );

  // The harness registry is the source of truth for the judge mode. Fall back
  // to the DB function_metadata for any question not yet registered.
  const profile = getProfile(question?.slug);
  const judgeMetadata = profile?.judge
    ? { ...(question?.function_metadata || {}), judge: profile.judge }
    : question?.function_metadata;
