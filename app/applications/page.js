            ...
            <div className="divide-y divide-white/5">
              {rows.map((row,index)=>{
                ...
                return <div key={index} className={`grid grid-cols-[42px_180px_220px_150px_150px_210px_220px_90px_180px] items-center gap-2 px-3 py-2 ${incomplete?'bg-red-500/5':''}`}>
                  ...
                  <input aria-label={`Date applied ${index+1}`} type="date" value={row.date_applied} onChange={e=>update(index,'date_applied',e.target.value)} className="rounded-lg border border-white/10 bg-slate-900 px-2 py-2 text-sm [color-scheme:dark]"/>
                  <select aria-label={`Status ${index+1}`} value={row.status} onChange={e=>update(index,'status',e.target.value)} className="rounded-lg border border-white/10 bg-slate-900 px-2 py-2 text-sm">{STATUSES.map(([key,label])=><option key={key} value={key}>{label}</option>)}</select>
