...
// Numeric equality within LeetCode's tolerance, recursing into arrays. Used by
// the float_approx mode so floating-point round-off (2.1**3 -> 9.2610000000001)
// does not fail an otherwise-correct answer.
function approxEqual(a, b, eps = 1e-5) {
  if (typeof a === 'number' && typeof b === 'number') {
    return Math.abs(a - b) <= eps * (1 + Math.abs(b));
  }
  if (Array.isArray(a) && Array.isArray(b)) {
    return a.length === b.length && a.every((x, i) => approxEqual(x, b[i], eps));
  }
  return a === b;
}
...
export function judgeOutput({ slug, metadata, stdin, expectedStdout, actualStdout, exitCode = 0, stderr = '' }) {
  ...
  if (cfg.mode === 'float_approx') {
    if (actualParsed.ok && expectedParsed.ok) {
      return { passed: approxEqual(actualParsed.value, expectedParsed.value), mode: 'float_approx', normalizedActual: actualParsed.value, normalizedExpected: expectedParsed.value };
    }
    const a = normalizeLines(actualStdout);
    const e = normalizeLines(expectedStdout);
    return { passed: a === e, mode: 'float_approx', normalizedActual: a, normalizedExpected: e };
  }
  ...
  if (cfg.mode === 'exact') {
    ...
    // Defense in depth for string returns: exactly one side parsed as a JSON
    // string (e.g. expected '"BANC"') while the other is the raw unquoted token
    // (e.g. actual 'BANC'). Compare the parsed string value to the raw text.
    const aIsStr = actualParsed.ok && typeof actualParsed.value === 'string';
    const eIsStr = expectedParsed.ok && typeof expectedParsed.value === 'string';
    if (aIsStr !== eIsStr && (aIsStr || eIsStr)) {
      const left = aIsStr ? actualParsed.value : normalizeLines(actualStdout);
      const right = eIsStr ? expectedParsed.value : normalizeLines(expectedStdout);
      return { passed: left === right, mode: 'exact', normalizedActual: left, normalizedExpected: right };
    }
    const a = normalizeLines(actualStdout);
    const e = normalizeLines(expectedStdout);
